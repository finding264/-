...
test_loader = LoadSignalDataset("D:/作业/testA.csv", need_over_sample=False, test=True)
test_loader = DataLoader(test_loader, batch_size=batch_size, drop_last=False, shuffle=False)

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
model = torch.load('D:/作业/Complex_CNN-unbalanced.pth').to(device)
...
with torch.no_grad():
    ...
    for i in range(len(labels)):
        ...
        index += 1
    with open("D:/作业/submit.csv", "w") as csvfile:
